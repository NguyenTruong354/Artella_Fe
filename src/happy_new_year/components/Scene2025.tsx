...
const Scene2025: React.FC<Scene2025Props> = ({ onComplete }) => {
  ...
  useEffect(() => {
    ...
    // scene.background = new THREE.Color(0x000000); // Removed for trail effect

    const camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000);
    camera.position.z = 100;

    // Enable preserveDrawingBuffer for trail effect
    ...
    loader.load('https://threejs.org/examples/fonts/helvetiker_bold.typeface.json', (font: any) => {
      ...
      let animationPhase = 'forming'; // forming -> holding -> exploding
      ...
      const animate = () => {
        ...
        // Trail effect: Fade out previous frame manually
        renderer.autoClearColor = false;
        const fadeMaterial = new THREE.MeshBasicMaterial({
            color: 0x000000,
            transparent: true,
            opacity: 0.1 // Adjust for trail length (lower = longer trails)
        });
        const fadePlane = new THREE.Mesh(new THREE.PlaneGeometry(10000, 10000), fadeMaterial);
        fadePlane.position.z = -1; // Behind everything but in front of clear color
        // Actually, simple clear with opacity is better done via a full screen quad or just not clearing fully
        // But since we have a black background, we can just render a black quad with low opacity
        
        // Simple trail hack:
        // We need to render a semi-transparent black rectangle over the whole screen
        // But we are inside a component. 
        // Let's use the renderer's scissor test or just a big plane in front of camera?
        // Or simpler: just don't clear, and let the black background of the div handle it? 
        // No, we need to draw black on top.
        
        // Let's use a big black plane attached to camera
        if (!scene.getObjectByName('fadePlane')) {
            const plane = new THREE.Mesh(
                new THREE.PlaneGeometry(1000, 1000),
                new THREE.MeshBasicMaterial({ color: 0x000000, transparent: true, opacity: 0.15 })
            );
            plane.name = 'fadePlane';
            plane.position.z = -10; // In front of camera (camera is at 100)
            camera.add(plane);
            scene.add(camera);
        }
        ...
        if (animationPhase === 'holding' && time > 8.0) {
            ...
            for (let i = 0; i < particleCount; i++) {
                velocities[i * 3] = (Math.random() - 0.5) * 10; // Stronger Random X burst
                velocities[i * 3 + 1] = (Math.random() - 0.5) * 10 + 5; // Stronger Initial Upward burst
                velocities[i * 3 + 2] = (Math.random() - 0.5) * 10; // Stronger Random Z burst
            }
        }
        ...
        if (animationPhase !== 'exploding') {
            ...
            particles.rotation.x = Math.cos(time * 0.1) * 0.1;
             ...
             if (material.opacity > 0) {
                 material.opacity -= 0.005; // Faster fade out
             } else {
        ...
        for (let i = 0; i < particleCount; i++) {
          ...
          if (animationPhase === 'exploding') {
             // Gravity
             velocities[i * 3 + 1] -= 0.2; // Stronger Gravity down
