...
import { useReducedMotion } from '../../hooks/useReducedMotion'; // Corrected import
...
const REDUCED_MOTION_TRANSITION = { type: "tween", duration: BASE_TRANSITION_DURATION };
...
const HEADER_ICON_ROTATION_TRANSITION = { duration: 20, repeat: Infinity, ease: "linear" };
const HEADER_ICON_GLOW_ANIMATION = { scale: [1, 1.2, 1], opacity: [0.5, 0.8, 0.5] };
...
const TOGGLE_BUTTON_GLOW_ANIMATION = { scale: [1, 1.2, 1], opacity: [0.3, 0.6, 0.3] };
...
const Sidebar: React.FC<SidebarProps> = ({ isOpen, onToggle }) => {
  ...
  const sidebarVariants = {
    open: {
      x: 0,
      transition: prefersReducedMotion ? REDUCED_MOTION_TRANSITION : SPRING_TRANSITION_NORMAL
    },
    closed: {
      x: "-100%",
      transition: prefersReducedMotion ? REDUCED_MOTION_TRANSITION : SPRING_TRANSITION_SIDEBAR_CLOSED
    }
  };
  ...
  const itemVariants = {
    open: {
      opacity: 1,
      x: 0,
      transition: prefersReducedMotion ? REDUCED_MOTION_TRANSITION : SPRING_TRANSITION_ITEM_OPEN
    },
    closed: {
      opacity: 0,
      x: -30,
      transition: prefersReducedMotion ? REDUCED_MOTION_TRANSITION : { duration: SPRING_TRANSITION_ITEM_CLOSED_DURATION }
    }
  };
  ...
  return (
    <>
      ...
      <motion.aside
        ...
        style={{ willChange: 'transform' }} // Added will-change
      >
        {/* Modern Background */}
          ...
          <div className="absolute inset-0">
            {[...Array(5)].map((_, i) => ( // Changed from 8 to 5
              <motion.div
                ...
                animate={prefersReducedMotion ? REDUCED_FLOATING_PARTICLE_ANIMATION : FLOATING_PARTICLE_ANIMATION}
                transition={prefersReducedMotion ? REDUCED_MOTION_TRANSITION : FLOATING_PARTICLE_TRANSITION()}
              />
            ))}
          </div>
        ...
        <div className="relative z-10 flex flex-col h-full">
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <div className="flex items-center space-x-4">
                <motion.div 
                  ...
                  transition={prefersReducedMotion ? REDUCED_MOTION_TRANSITION : HEADER_ICON_ROTATION_TRANSITION}
                  style={{ willChange: 'transform' }} // Added will-change
                >
                  <div className="w-12 h-12 bg-gradient-to-br from-blue-500 to-cyan-500 dark:from-amber-500 dark:to-orange-500 rounded-2xl flex items-center justify-center shadow-lg relative overflow-hidden">
                    <motion.div
                      ...
                      animate={prefersReducedMotion ? { scale: 1, opacity: 0.5 } : HEADER_ICON_GLOW_ANIMATION}
                      transition={prefersReducedMotion ? REDUCED_MOTION_TRANSITION : HEADER_ICON_GLOW_TRANSITION}
                      style={{ willChange: 'transform, opacity' }} // Added will-change
                    />
                  ...
                  <motion.div 
                    ...
                    animate={prefersReducedMotion ? { scale: 1 } : HEADER_ICON_DOT_ANIMATION}
                    transition={prefersReducedMotion ? REDUCED_MOTION_TRANSITION : HEADER_ICON_DOT_TRANSITION}
                    style={{ willChange: 'transform' }} // Added will-change
                  />
                </motion.div>
                <div>
            ...
            <motion.div 
              ...
              animate="animate" // This will now use the conditional animation from floatingVariants
              style={{ willChange: 'transform, opacity' }} // Added will-change
            >
          ...
          <motion.div 
            ...
            style={{ willChange: 'opacity' }} // Added will-change for children opacity
          >
            <div>
              <h3 id="navigation-heading" className="text-gray-500 dark:text-gray-400 text-xs font-bold uppercase tracking-widest mb-4 flex items-center">
                    ...
                    >
                      ...
                      {activeSection === item.path && (
                        <motion.div
                          ...
                          transition={prefersReducedMotion ? REDUCED_MOTION_TRANSITION : SPRING_TRANSITION_ACTIVE_INDICATOR}
                          style={{ willChange: 'transform, opacity' }} // Added will-change
                        />
                      )}
                      ...
                      <motion.div // Changed from div to motion.div
                        ...
                        whileHover={prefersReducedMotion ? {} : { scale: 1.15, rotate: 5 }} // Added micro-animation
                        transition={prefersReducedMotion ? REDUCED_MOTION_TRANSITION : SPRING_TRANSITION_ICON_HOVER} // Added transition for animation
                        style={{ willChange: 'transform' }} // Added will-change
                      >
            ...
            <div>
              <h3 id="quick-actions-heading" className="text-gray-500 dark:text-gray-400 text-xs font-bold uppercase tracking-widest mb-4 flex items-center">
                <span className="w-8 h-px bg-gradient-to-r from-blue-400 to-cyan-500 dark:from-amber-400 dark:to-orange-500 mr-3"></span>
                Quick Actions
              </h3>
                ...
                {quickActions.map((action, index) => (
                  ...
                  >
                    ...
                    <motion.div
                      ...
                      animate={prefersReducedMotion ? { opacity: 0 } : QUICK_ACTION_HIGHLIGHT_ANIMATION() }
                      transition={prefersReducedMotion ? REDUCED_MOTION_TRANSITION : QUICK_ACTION_HIGHLIGHT_TRANSITION(index)}
                      style={{ willChange: 'transform, opacity' }} // Added will-change
                    />
      ...
      <motion.button
        ...
        animate={isOpen 
            ? (prefersReducedMotion ? { x: 320, rotate: 0 } : { x: 320, rotate: 180 }) 
            : { x: 0, rotate: 0 }}
        transition={prefersReducedMotion ? REDUCED_MOTION_TRANSITION : SPRING_TRANSITION_TOGGLE_BUTTON}
        style={{ willChange: 'transform' }} // Added will-change
      >
        <motion.div
          className="absolute inset-0 bg-gradient-to-br from-white/30 to-transparent"
          animate={prefersReducedMotion ? { scale: 1, opacity: 0.3 } : TOGGLE_BUTTON_GLOW_ANIMATION}
          transition={prefersReducedMotion ? REDUCED_MOTION_TRANSITION : TOGGLE_BUTTON_GLOW_TRANSITION}
          style={{ willChange: 'transform, opacity' }} // Added will-change
        />
